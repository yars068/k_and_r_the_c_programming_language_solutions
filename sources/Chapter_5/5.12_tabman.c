...
int detab(char *buf, int tabsize, int tab_start_pos);
int entab(char *buf, int tabsize, int tab_start_pos);
...
int main(int argc, char *argv[]) {
  ...
  int def_tab_start = TRUE; /* tab stop starts from beginnig of line */
  int tab_start_pos = 0;
  ...
  while (argc-- != NO_ARGS && *++argv) { /* Skip *argv[0] because this points to program name */
    while (**argv == '-' || **argv == '+') {
      if (**argv == '+') {
        def_tab_start = FALSE;
        if (argc != NO_ARGS) {
          printf("tabman: Error: too many arguments\n");
          return EXIT_FAILURE;
        }
      }
      switch (*++*argv) {
  ...
  stops = stops_arr;
  if (!def_tab_size && !def_tab_start) {
    tab_start_pos = *stops++;
    tabsize = *stops;
    printf("tabman: tab stop first position is %d, tab stop size is %d\n", tab_start_pos, tabsize);
  }
  ...
  stops = stops_arr;
  flag = FALSE;
  while (!def_tab_size && def_tab_start && *++stops) {
    if (*stops - *(stops - 1) == tabsize) flag = TRUE;
    else flag = FALSE;
  }
  if (!flag && !def_tab_size && def_tab_start) {
  ...
  if (ask_line(buf, BUFSIZE, msg)) {
    switch (mode) {
      case DETAB:
        if (detab(buf, tabsize, tab_start_pos)) flag = TRUE;
        break;
      case ENTAB:
        if (entab(buf, tabsize, tab_start_pos)) flag = TRUE;
        break;
    }
  }
...
int detab(char *buf, int tabsize, int tab_start_pos) {
  ...
  while (*tmp) {
    /* keep input data unchanged before tab_start_pos */
    if (*tmp != TAB || ((tmp - tmpbuf) <= tab_start_pos)) {
      *buf++ = *tmp++;
      len++;
    }
    else {
...
int entab(char *buf, int tabsize, int tab_start_pos) {
  ...
  while (*tmp) {
    /* keep input data unchanged before tab_start_pos */
    if (*tmp != SPACE || ((tmp - start) <= tab_start_pos)) *buf++ = *tmp++;
    else {
