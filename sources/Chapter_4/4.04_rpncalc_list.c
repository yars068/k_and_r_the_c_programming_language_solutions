...
/* Lesson 4.4. Add commands to print the top of stack (without popping), */
/* to duplicate it, and to swap the two top elements. Add command to clear stack */
  ...
  while ((type = getop (s)) != EOF) {
    switch (type) {
      ...
      case 'd': case 'D': /* duplicate */
        ...
        push(op2);
        break;
      case 's': case 'S': /* swap operands, simplest way */
        op2 = pop();
        op1 = pop();
        push(op2);
        push(op1);
